# Smart Home AI Assistant# Trợ Lý Thông Minh Nhà Thông Minh# Smart Home AI Assistant



Trợ lý AI thông minh chạy hoàn toàn local trên MacBook, sử dụng camera và microphone tích hợp.



## Kiến trúcMột trợ lý AI cho macOS (Apple Silicon) sử dụng camera và microphone tích hợp của MacBook để tạo trải nghiệm AI đa phương thức.An AI assistant for macOS (Apple Silicon) that uses your MacBook's built-in camera and microphone to create a multimodal AI experience.



Dự án được xây dựng theo Clean Architecture với Go, bao gồm:



- **Domain Layer**: Định nghĩa entities và repository interfaces## 🎯 Chức Năng## 🎯 What It Does

- **Use Case Layer**: Orchestration logic cho luồng xử lý AI

- **Infrastructure Layer**: Adapters cho các dịch vụ bên ngoài (FFmpeg, Whisper.cpp, Ollama, MacTTS)

- **Delivery Layer**: Entry point (cmd/assistant)

1. **Bắt ảnh** từ camera FaceTime HD1. **Captures** a single frame from your FaceTime HD camera

## Stack công nghệ (Local)

2. **Thu âm** 5 giây từ microphone tích hợp2. **Records** 5 seconds of audio from your built-in microphone

- **Ngôn ngữ**: Go 1.22+

- **Camera/Microphone**: FFmpeg với AVFoundation3. **Chuyển đổi giọng nói** thành văn bản bằng OpenAI Whisper API3. **Transcribes** your speech using OpenAI Whisper API

- **Speech-to-Text**: whisper.cpp (local)

- **AI Assistant**: Ollama (phi-3-mini hoặc model nhẹ khác)4. **Phân tích** hình ảnh và văn bản cùng nhau bằng GPT-4o-mini4. **Analyzes** the image and text together using GPT-4o-mini
...
- macOS (để sử dụng AVFoundation và `say` command)

- Go 1.22 trở lên

- FFmpeg với hỗ trợ AVFoundation

- Ollama CLI## 🧰 Yêu Cầu## 🧰 Prerequisites
...
brew install ffmpeg### 2. Cài đặt ffmpeg và ffplay### 2. Install ffmpeg and ffplay
...
- `gemma:2b` (2B parameters) - nhỏ gọn, hiệu suất tốt
...
# Clone repository- Tạo API key mới- Create a new API key

git clone https://github.com/ggerganov/whisper.cpp.git

cd whisper.cpp- Thiết lập biến môi trường:- Set it as an environment variable:
...
# Whisper.cpp configuration## 🚀 Cài Đặt```
...
## Chạy ứng dụnggo mod tidy**Note:** The project now uses Clean Architecture. See [ARCHITECTURE.md](ARCHITECTURE.md) for details.
...
# Hoặc dùng go run## ▶️ Chạy Ứng Dụng# Run directly
...
## Luồng hoạt động**Lưu ý:** Dự án sử dụng Clean Architecture. Xem [KY_THUAT.md](KY_THUAT.md) để biết chi tiết.



1. **Chụp ảnh từ camera** - FFmpeg capture một frame từ camera tích hợp# Or build binary

2. **Ghi âm câu hỏi** - FFmpeg ghi âm 5 giây từ microphone

3. **Chuyển giọng nói thành text** - whisper.cpp transcribe file audio```bashgo build -o smart-home-ai cmd/assistant/main.go

4. **Phân tích đa phương thức** - Ollama xử lý text + ảnh

5. **Phát giọng nói phản hồi** - macOS `say` đọc câu trả lời# Chạy trực tiếp./smart-home-ai



## Cấu trúc thư mụcgo run cmd/assistant/main.go```
...
├── cmd/# Hoặc build file thực thi### Expected Output:

│   └── assistant/

│       └── main.go              # Entry pointgo build -o tro-ly-thong-minh cmd/assistant/main.go```

├── internal/

│   ├── domain/./tro-ly-thong-minh[15:04:05] 🎥 Capturing image from FaceTime HD camera...

│   │   ├── entity.go            # Domain entities

│   │   └── repository.go        # Repository interfaces (ports)```[15:04:06] ✅ Image captured successfully
...
│       ├── media/### Kết Quả Mong Đợi:[15:04:11] ✅ Audio recorded successfully
...
│       ├── ollama/[15:04:05] 🎥 Đang bắt ảnh từ camera FaceTime HD...[15:04:12] 📝 Transcription: "What do you see in front of me?"

│       │   └── client.go        # Ollama adapter

│       └── mactts/[15:04:06] ✅ Đã bắt ảnh thành công[15:04:12] 🤖 GPT-4o-mini reasoning on text + image...

│           └── synthesizer.go   # MacTTS adapter

├── pkg/[15:04:06] 🎤 Đang thu âm từ microphone (5 giây)...[15:04:14] 💬 GPT Response: "I can see you're sitting at a desk with a laptop..."

│   └── logger/

│       └── console.go           # Console logger[15:04:11] ✅ Đã thu âm thành công[15:04:14] 🔊 Converting response to speech with TTS...

├── go.mod

├── go.sum[15:04:11] 🧠 Đang chuyển giọng nói thành văn bản với Whisper API...[15:04:16] ✅ Speech generated successfully
